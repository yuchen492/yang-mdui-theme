    ...
    <div class="mdui-col">
        <div class="mdui-card">
          <div class="mdui-card-primary">
            <div class="mdui-card-primary-title">虚拟主机</div>
          </div>
          <div class="mdui-card-content">
            <p>{if $CustomDepot.yangmdui_clientname}{$CustomDepot.yangmdui_clientname}{else}{$Setting.company_name}{/if}提供了多种虚拟主机产品。</p>
            <p>您可在{if $CustomDepot.yangmdui_clientname}{$CustomDepot.yangmdui_clientname}{else}{$Setting.company_name}{/if}管理您的 EasyPanel 虚拟主机、Bthost 虚拟主机。{if $CustomDepot.yangmdui_clientname}{$CustomDepot.yangmdui_clientname}{else}{$Setting.company_name}{/if}提供了直接管理和离站管理多种架构，帮助您实践最佳业务架构。</p>
          </div>
        </div>
    </div>
</div>
    	...
    	{foreach $Service.list as $list}
    	    <div class="mdui-col">
        	    ...
        	    <div class="mdui-card mdui-hoverable mdui-m-y-1">
                  <div class="mdui-card-primary">
                    <div class="mdui-card-primary-title mdui-text-truncate"{if $list.notes} mdui-tooltip="{content: '您的备注：{$list.notes}'}"{/if}>
                        {if $list.notes}
                            {$list.notes}
                        {else}
                            我的虚拟主机实例 <i class="mdui-icon material-icons mdui-text-color-blue" mdui-tooltip="{content: '此实例没有备注，您可以前往产品内页设置备注，这样您的备注就可以清晰地展现在卡片上。'}">help_outline</i>
                        {/if}
                  ...
                  <div class="mdui-card-content">
                    <div class="dots" id="service{$list.id}" style="display:none" data-toggle="tooltip" data-placement="top" onclick="getSingleStatus('{$list.id}')">
					</div>
                    ...
                    <span><i class="mdui-icon material-icons">apps</i> 产品 ID: {$list.id}</span><br>
                        {if $list.site_max}<span><i class="mdui-icon material-icons">folder</i> 空间：{$list.site_max}</span><br>{/if}
                        {if $list.space}<span><i class="mdui-icon material-icons">folder</i> 空间：{$list.space}</span><br>{/if}
                        {if $list.web_quota}<span><i class="mdui-icon material-icons">folder</i> 空间：{$list.web_quota}</span><br>{/if}
                        {if $list.webdx}<span><i class="mdui-icon material-icons">folder</i> 空间：{$list.webdx}</span><br>{/if}
                        {if $list.sql_max}<span><i class="mdui-icon material-icons">find_in_page</i> 数据库：{$list.sql_max}</span><br>{/if}
                        {if $list.db_quota}<span><i class="mdui-icon material-icons">find_in_page</i> 数据库：{$list.db_quota}</span><br>{/if}
                        {if $list.db}<span><i class="mdui-icon material-icons">find_in_page</i> 数据库：{$list.db}</span><br>{/if}
                        {if $list.sqldx}<span><i class="mdui-icon material-icons">find_in_page</i> 数据库：{$list.sqldx}</span><br>{/if}
                        {if $list.flow_max}<span><i class="mdui-icon material-icons">backup</i> 月流量：{$list.flow_max}</span><br>{/if}
                        {if $list.flow_limit}<span><i class="mdui-icon material-icons">backup</i> 月流量：{$list.flow_limit}</span><br>{/if}
                        {if $list.bandwidth}<span><i class="mdui-icon material-icons">backup</i> 月流量：{$list.bandwidth}</span><br>{/if}
                        {if $list.broadband_up}<span><i class="mdui-icon material-icons">arrow_upward</i> {$list.broadband_up} / <i class="mdui-icon material-icons">arrow_downward</i> {$list.broadband_down}</span><br>{/if}
                        {if $list.speed}<span><i class="mdui-icon material-icons">arrow_upward</i> {$list.speed} / <i class="mdui-icon material-icons">arrow_downward</i> {$list.speed}</span><br>{/if}
                    <span><i class="mdui-icon material-icons">access_time</i> 到期日期：{$list.nextduedate|date="Y-m-d"}</span><br>
