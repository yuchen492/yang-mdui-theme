    ...
    <div class="mdui-col">
        <div class="mdui-card">
          ...
          <div class="mdui-card-content">
            <p>{if $CustomDepot.yangmdui_clientname}{$CustomDepot.yangmdui_clientname}{else}{$Setting.company_name}{/if}提供了 KVM 高度虚拟化的云服务器产品。</p>
            <p>您可在{if $CustomDepot.yangmdui_clientname}{$CustomDepot.yangmdui_clientname}{else}{$Setting.company_name}{/if}管理您的云服务器产品，依托本平台快速构建云端服务架构。</p>
          </div>
        </div>
    </div>
</div>

{include file="includes/product_manage_status_select"}
...
{if $Service.list}
    	...
    	{foreach $Service.list as $list}
    	    ...
    	    <div class="mdui-col">
        	    ...
        	    <div class="mdui-card mdui-hoverable mdui-m-y-1">
                  <div class="mdui-card-primary">
                    <div class="mdui-card-primary-title"{if $list.notes} mdui-tooltip="{content: '您的备注：{$list.notes}'}"{/if}>
                        {if $list.notes}
                            {$list.notes}
                        {else}
                            我的云服务器 <i class="mdui-icon material-icons mdui-text-color-blue" mdui-tooltip="{content: '此实例没有备注，您可以前往产品内页设置备注，这样您的备注就可以清晰地展现在卡片上。'}">help_outline</i>
                        {/if}
                  ...
                  <div class="mdui-card-content">
                    ...
                    <span><i class="mdui-icon material-icons">apps</i> 产品 ID: {$list.id}</span><br>
                        ...
                        <span><i class="mdui-icon material-icons">dns</i> 主 IP：
                            {if $list.dedicatedip}
                                {$list.dedicatedip}
                            {else}
                                无独立 IP <a href="javascript:;" mdui-tooltip="{content: '实例未绑定独立 IP 意味着您的实例无法联通公网或与其他用户共享使用同一个公网 IP。购买独立 IP 可享受更快和更安全的访问体验。'}">(这会影响什么?)</a>
                            {/if}
                        </span><br>
                    ...
                    <span>
                        CPU使用率：<span id="yang-mdui-cpu-lable-{$list.id}">获取中</span>
                        <div class="mdui-progress">
                            <div id="yang-mdui-cpu-progress-{$list.id}" class="mdui-progress-indeterminate"></div>
                        </div>
                    </span>
                    <span>
                        内存使用率：<span id="yang-mdui-mem-lable-{$list.id}">获取中</span>
                        <div class="mdui-progress">
                            <div id="yang-mdui-mem-progress-{$list.id}" class="mdui-progress-indeterminate"></div>
                        </div>
                    </span>
                    <script>
                    $.ajax({
                        ...
                        success: function(data) {
                            if (cpuData = data.data.list[0]) {
                              ...
                              $("#yang-mdui-cpu-progress-{$list.id}").css("width",cpuDataLatest + "%")
                              $("#yang-mdui-cpu-lable-{$list.id}").html(cpuDataLatest + "%")
                              if (cpuDataLatest > 85) {
                                $("#yang-mdui-cpu-progress-{$list.id}").addClass('mdui-color-red')
                              } else if (cpuDataLatest > 65) {
                                $("#yang-mdui-cpu-progress-{$list.id}").addClass('mdui-color-orange')
                              } else {
                                $("#yang-mdui-cpu-progress-{$list.id}").addClass('mdui-color-green')
                              }
                            } else {
                              $("#yang-mdui-cpu-progress-{$list.id}").css("width","0%")
                              $("#yang-mdui-cpu-lable-{$list.id}").html("暂无数据，请稍后再试")
                            }
                              $("#yang-mdui-cpu-progress-{$list.id}").removeClass('mdui-progress-indeterminate')
                              $("#yang-mdui-cpu-progress-{$list.id}").addClass('mdui-progress-determinate')
                            },
                        error: function(data) {
                            $("#yang-mdui-cpu-progress-{$list.id}").css("width","0%")
                            $("#yang-mdui-cpu-lable-{$list.id}").html("暂无数据，请稍后再试")
                    ...
                    <script>
                        ...
                        success: function(data) {
                            ...
                            $("#yang-mdui-mem-progress-{$list.id}").css("width",memDisplay + "%")
                            $("#yang-mdui-mem-lable-{$list.id}").html(memnowDataLatest + memUnit + " / " + memallDataLatest + memUnit + " (" + Math.round(memDisplay) + "%)")
                            if (memDisplay > 75) {
                                $("#yang-mdui-mem-progress-{$list.id}").addClass('mdui-color-red')
                            } else if (memDisplay > 60) {
                                $("#yang-mdui-mem-progress-{$list.id}").addClass('mdui-color-orange')
                            } else {
                                $("#yang-mdui-mem-progress-{$list.id}").addClass('mdui-color-green')
                            }
                            $("#yang-mdui-mem-progress-{$list.id}").removeClass('mdui-progress-indeterminate')
                            $("#yang-mdui-mem-progress-{$list.id}").addClass('mdui-progress-determinate')
                        },
                        error: function(data) {
                            $("#yang-mdui-mem-progress-{$list.id}").css("width","0%")
                            $("#yang-mdui-mem-lable-{$list.id}").html("暂无数据，请稍后再试")
                            $("#yang-mdui-mem-progress-{$list.id}").removeClass('mdui-progress-indeterminate')
                            $("#yang-mdui-mem-progress-{$list.id}").addClass('mdui-progress-determinate')
                        }
                        });
                    ...
                    {else}
                    <span>
                        CPU使用率：<span id="yang-mdui-cpu-lable-{$list.id}">不能获取未激活的实例用量</span>
                        <div class="mdui-progress">
                            <div id="yang-mdui-cpu-progress-{$list.id}" style="width: 0%;" class="mdui-progress-determinate"></div>
                        </div>
                    </span>
                    <span>
                        内存使用率：<span id="yang-mdui-mem-lable-{$list.id}">不能获取未激活的实例用量</span>
                        <div class="mdui-progress">
                            <div id="yang-mdui-mem-progress-{$list.id}" style="width: 0%;" class="mdui-progress-determinate"></div>
                        </div>
                    </span>
                    {/if}
                    
                  </div>
                  {include file="includes/product_manage_actions"}
                </div>
    ...
    {include file="includes/product_manage_pages"}
{else}
	<div class="mdui-card mdui-m-y-1">
      ...
      <div class="mdui-card-content">
        <p>千里之行，始于足下。</p>
        <p>{if $CustomDepot.yangmdui_teamname}{$CustomDepot.yangmdui_teamname}{else}{$Setting.company_name}{/if}不同用户中心间产品数据不互通，另有说明的产品除外。请您检查产品下单位置。</p>
      </div>
    </div>
{/if}
