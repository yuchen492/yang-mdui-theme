    ...
    <div class="mdui-col">
        <div class="mdui-card">
          <div class="mdui-card-primary">
            <div class="mdui-card-primary-title">内容分发网络与路由</div>
          </div>
          <div class="mdui-card-content">
            <p>{if $CustomDepot.yangmdui_clientname}{$CustomDepot.yangmdui_clientname}{else}{$Setting.company_name}{/if}提供了 CDN 实例。</p>
            <p>您可{if $CustomDepot.yangmdui_clientname}{$CustomDepot.yangmdui_clientname}{else}{$Setting.company_name}{/if}管理您的 CDN，快速上线加速与源站保护服务。</p>
          </div>
        </div>
    </div>
</div>
    	    ...
    	    <div class="mdui-col">
        	    ...
        	    <div class="mdui-card mdui-hoverable mdui-m-y-1">
                  <div class="mdui-card-primary">
                    <div class="mdui-card-primary-title"{if $list.notes} mdui-tooltip="{content: '您的备注：{$list.notes}'}"{/if}>
                        {if $list.notes}
                            {$list.notes}
                        {else}
                            我的 CDN 实例 <i class="mdui-icon material-icons mdui-text-color-blue" mdui-tooltip="{content: '此实例没有备注，您可以前往产品内页设置备注，这样您的备注就可以清晰地展现在卡片上。'}">help_outline</i>
                        {/if}
                  ...
                  <div class="mdui-card-content">
                    ...
                    {if $list.domain}<span><i class="mdui-icon material-icons">link</i> 授权域名数：{$list.domain}</span><br>{/if}
                    {if $list.flow_limit}<span><i class="mdui-icon material-icons">backup</i> 月流量：{$list.flow_limit}</span><br>{/if}
                    {if $list.speed_limit}<span><i class="mdui-icon material-icons">arrow_upward</i> 出网宽带： {$list.speed_limit}</span><br>{/if}
...
{else}
	<div class="mdui-card mdui-m-y-1">
      <div class="mdui-card-primary">
        <div class="mdui-card-primary-title">您尚未开通任何 CDN 实例</div>
      </div>
      <div class="mdui-card-content">
        <p>千里之行，始于足下。</p>
